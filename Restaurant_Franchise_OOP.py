...
class Franchise:
  def __init__(self, adress, menus):
    ...
    self.menus = menus
  def __repr__(self):
    return 'Welcome to our new restaurant at ' + self.adress + ' , these are our avalabile menus: ' + str(self.menus)
  def avalabile_menus(self, time):
    x = []
...
class Menu:
  ...
  def __repr__(self):
    return self.name + ' is avalabile from ' + str(self.start_time) + ' to ' + str(self.end_time)
  ...
  def calculate_bill(self, purchased_items):
    ...
    for purchased_item in purchased_items:
      if purchased_item in self.items:
        ...
        print(self.name + ' item not in menu')
    return bill
...
early_bird_menu = Menu('Early_Bird menu', early_bird_items, 15, 18 )
...
franchises.append(arepas_place
)
